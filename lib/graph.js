...
function Graph( text, directed ) {
	...
	this.minimumSpanningTree = function() {
		...
		for( var edge of sortedEdges ) {
			var u = mst.addNodeIfNotPresent( edge.from.name ) ;
			var v = mst.addNodeIfNotPresent( edge.to.name ) ;
...
//				console.log( "edge from ", u.name , 'to', v.name ) ;
				...
				var setId = Math.min( u.mstSet, v.mstSet ) ;
				mst.reset() ;
				u.bfs( function(n) { n.mstSet = setId ; } ) ;
			}
		}
